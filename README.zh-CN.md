...
完全相同的 clipping 重复副本只写一次。新记录使用完整 SHA-256 `kls2-...` 身份，因此不同 clipping 即使旧的 32 位 `kls-...` ID 发生碰撞，也会保持独立。

旧笔记和已保存选择只会在旧 ID 及其物理区块或状态唯一匹配当前 clipping 后惰性迁移。如果同一本书内的旧 ID 存在歧义，插件会保持该书及其已保存决定不变，并在摘要中说明冲突；不相关书籍仍可继续。

如果旧碰撞中当前只出现一条标注，插件无法知道早先的 Import 或 Ignore 选择实际指向哪一条。插件会保留旧证据；如果第二条标注之后出现并使冲突可见，该书笔记会保持不变并等待审核。

降级警告：`0.1.2` 及更早版本不具备碰撞安全性，也不理解新的权威身份。不要用旧版本重写或清理由此版本写入的笔记或状态。

## 已知限制

存在歧义的旧版冲突会被安全隔离；引导式恢复功能将延后提供。
