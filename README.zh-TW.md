...
同一 clipping 的重複副本只寫入一次。身分還包括精確書名和作者，因此兩本不同書即使產生相同 ID，也不會混用已儲存選擇。

已知限制：ID 使用 32 位元雜湊。同一精確書名與作者下的兩個不同 clipping 仍可能碰撞並被當作同一條。此問題尚未解決；在發行版本宣稱完整碰撞安全前，必須測試或修正。
